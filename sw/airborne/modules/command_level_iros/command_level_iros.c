...
switch (state_lower_level)
{
		...
		case SET_THETA_CM:
            set_theta(-10.0/180.0*3.14);//this function is not in 'flight plan in guided mode'
            //set_phi(0.004);
            if (time_primitive > 5) {
			previous_lower_level = SET_THETA_CM;
                    state_lower_level = HOVER_CM;		
			}
			break;
		default:
